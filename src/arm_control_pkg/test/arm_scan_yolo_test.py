...
# First test with this small range. After checking cable clearance and camera
# coverage, this can be expanded gradually, for example to [-120, -60, 0, 60, 120].
SCAN_OFFSETS_DEG = [
    -140,
    -100,
    -60,
...
def show_one_scan_view(model, cap, offset_deg):
    """Show live recognition at one arm pose for VIEW_TIME_SEC seconds."""
    ...
    while time.monotonic() < deadline:
        ...
        for class_name, count in counts.items():
            maximum_counts[class_name] = max(
                maximum_counts[class_name],
                count,
            )

        cv2.imshow("OMX Pumpkin Scan", annotated)
...
def main():
    ...
    try:
        ...
        # Keep joints 2-5 at the ready pose and rotate only joint1 relative
        # to its ready angle.
        ...
        for offset_deg in SCAN_OFFSETS_DEG:
            ...
            # The OMX material gives joint1 an approximate -150 to +150 deg
            # range. Keep a 5-degree margin from both limits.
            ...
            counts, stop_requested = show_one_scan_view(
                model,
                cap,
                offset_deg,
            )
