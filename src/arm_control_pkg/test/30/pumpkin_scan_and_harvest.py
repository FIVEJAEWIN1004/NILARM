"""Scan zones B/A, count, and repeatedly harvest into a taught front bin.
...
This version deliberately keeps the coordinate conversion and grasp motion
from the hardware-tested program.  Zone B is the negative-Y half and zone A
is the positive-Y half of the robot base frame.  No alternative pitch search
or A/B homography rotation is performed before harvesting.
"""
...
def target_zone(target):
    """Return the competition zone from the calibrated robot-base Y axis."""
    return "A" if float(target.y) >= 0.0 else "B"


def print_ab_summary(targets):
    a_targets = [target for target in targets if target_zone(target) == "A"]
    b_targets = [target for target in targets if target_zone(target) == "B"]

    print("\n========== A/B 호박 개수 ==========")
    print(f"A-{len(a_targets)} / B-{len(b_targets)}")
    for index, target in enumerate(targets, start=1):
        print(
            f"{index:02d}. {target_zone(target)}구역 "
            f"xy=({target.x:.3f}, {target.y:.3f}) m, "
